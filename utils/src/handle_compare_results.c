 ...
 *
 * This function iterates over the host and device arrays, comparing corresponding elements. If a
 * mismatch is detected, it prints an error message to stdout with details on the location of the
 * mismatch.
 *
 * The function uses relative tolerance (rtol) and absolute tolerance (atol) values that are defined
 * based on the type being used (MYTYPE). These tolerances are scaled to be around 5-10 times the
 * epsilon value for the respective type.
 *
 ...
 */
void
compareHostDevice( const MYTYPE *Host, const MYTYPE *Device, const int rows, const int cols )
{
    int mismatches = 0;
// Type-specific tolerances (scale ~5-10× epsilon)
    ...
    for ( int i = 0; i < rows; i++ ) {
        ...
        for ( int j = 0; j < cols; ++j ) {
            int k = j + offset;
            if ( fabs( Host[k] - Device[k] ) >
                 rtol * ( fabs( Host[k] ) + fabs( Device[k] ) + atol ) ) {
                printf( "mismatch (%d,%d): host=%.17g device=%.17g rel_err=%g\n",
                        i,
                        j,
                        Host[k],
                        Device[k],
                        fabs( Host[k] - Device[k] ) / ( fabs( Host[k] ) + atol ) );
                mismatches++;
                if ( mismatches >= 10 ) {
                    printf( "ABORT: >10 mismatches\n" );
                    return;
                }
            }
        }
 ...
 */
void
checkTranspose( const MYTYPE *input, MYTYPE *output, int rows, int cols )
{
    int mismatches = 0;
// Type-specific tolerances (scale ~5-10× epsilon)
#if defined( MYTYPE_FLOAT )
    ...
    for ( int i = 0; i < rows; i++ ) {
        ...
        for ( int j = 0; j < cols; j++ ) {
            ...
            if ( fabs( input[in_ID] - output[out_ID] ) >
                 rtol * ( fabs( input[in_ID] ) + fabs( output[out_ID] ) + atol ) ) {
                printf( "mismatch (%d,%d): host=%.17g device=%.17g rel_err=%g\n",
                        i,
                        j,
                        input[in_ID],
                        output[out_ID],
                        fabs( input[in_ID] - output[out_ID] ) / ( fabs( input[in_ID] ) + atol ) );
                mismatches++;
                if ( mismatches >= 10 ) {
                    printf( "ABORT: >10 mismatches\n" );
                    return;
